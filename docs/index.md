...
<!-- 
  ...
  1. Quicksand: 用于 Hello I'm (圆润字体)
  2. Pacifico: 用于名字 (艺术手写体)
  =========================================
-->
...
<div class="profile-container">
    ...
    <div class="right-content">
        ...
        <!-- 3. GitHub 贡献图 (已去除标题，极简风格) -->
        <div class="glass-box github-chart-box">
            <a href="https://github.com/4utumny" target="_blank" style="display: block;">
                <!-- 08e39a 是主题绿色 -->
                <img src="https://ghchart.rshah.org/08e39a/4utumny" alt="4utumny's Github Chart" class="gh-chart-img">
            </a>
        </div>
...
<style>
    ...
    #bg-overlay {
        ...
        background: rgba(0, 0, 0, 0.25); /* 稍微加深一点背景遮罩，突出白字 */
    }

    /* 布局容器 */
    .profile-container {
        position: relative; z-index: 2;
        display: flex; max-width: 1100px; margin: 8vh auto; gap: 30px; padding: 20px;
        font-family: 'Quicksand', 'Segoe UI', sans-serif; /* 默认使用 Quicksand 圆体 */
        color: #fff;
    }
    ...
    .info-row { display: flex; align-items: center; gap: 10px; font-size: 0.95rem; color: #e0e0e0; }
    
    .tags-box { display: flex; flex-wrap: wrap; gap: 8px; }
    .tag-badge {
        background: rgba(255, 255, 255, 0.15); padding: 4px 10px;
        border-radius: 8px; font-size: 0.8rem; color: #ddd;
    }

    .timeline-box { margin-top: auto; }
    .timeline-item { display: flex; gap: 15px; margin-bottom: 15px; position: relative; }
    .timeline-item:last-child { margin-bottom: 0; }
    .timeline-item:not(:last-child)::after {
        content: ''; position: absolute; left: 5px; top: 15px; bottom: -20px;
        width: 2px; background: rgba(255, 255, 255, 0.2);
    }
    .dot { width: 12px; height: 12px; background: #888; border-radius: 50%; margin-top: 5px; }
    ...
    /* ===== 右侧内容 ===== */
    .right-content { flex: 1; display: flex; flex-direction: column; gap: 30px; }
    .welcome-section { margin-top: 20px; }

    /* 优化后的标题字体 */
    .hello-text {
        font-family: 'Quicksand', sans-serif; /* 圆润字体 */
        font-size: 3.8rem;
        font-weight: 700;
    ...
    /* 名字高亮 - 艺术字 */
    .name-highlight {
        font-family: 'Pacifico', cursive; /* 艺术手写体 */
        font-weight: 400;
        padding-left: 10px;
        background: linear-gradient(120deg, #08e39a, #00b4d8);
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
        /* 为了防止渐变文字在深色背景看不清，加一点点滤镜 */
        filter: drop-shadow(0 2px 4px rgba(0,0,0,0.3));
    }

    .role-text { font-size: 1.2rem; margin: 10px 0; color: #e0e0e0; font-weight: 600; }
    .desc-text {
        font-size: 1rem; font-style: italic;
        background: rgba(0,0,0,0.3); padding: 10px 15px;
        border-radius: 8px; border-left: 4px solid #08e39a; display: inline-block;
    }
    .tag-highlight {
        background: rgba(8, 227, 154, 0.2); padding: 0 4px;
        border-radius: 4px; color: #08e39a;
    }
    ...
    .social-btn:hover {
        background: rgba(255,255,255,0.2); transform: translateY(-3px); color: #08e39a;
    }
    ...
    .gh-chart-img {
        ...
        display: block;
        border-radius: 4px;
        opacity: 0.9; /* 稍微透明一点，融入背景 */
    }

    .site-links-row { margin-top: auto; }
    .site-section-title {
    ...
    .link-card {
        background: rgba(40, 40, 40, 0.6); padding: 20px; border-radius: 16px;
    ...
    .footer-simple { margin-top: 20px; font-size: 0.8rem; color: rgba(255,255,255,0.5); }
    ...
    @media (max-width: 768px) {
        ...
        .hello-text { font-size: 2.8rem; } /* 移动端调小字体 */
